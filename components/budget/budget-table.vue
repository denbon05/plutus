<template>
  <section class="container is-fullhd">
    ...
    >
      ...
      <b-table-column v-slot="props" field="last_name" label="Last Name" :td-attrs="columnTdAttrs">
        {{ props.row.last_name }}
      </b-table-column>
      ...
      <b-table-column v-slot="props" label="Gender" :td-attrs="columnTdAttrs">
        <span>
          <b-icon
...
export default {
  ...
  computed: {
    monthBudgetNotes() {
      return Array.from({ length: moment('2020-02').daysInMonth() }, (_n, i) =>
  ...
  watch: {
    queryBudgetState: {
      deep: true,
      handler() {
        const { isSuccess, message, isLoading } = this.queryBudgetState;
        if (!isSuccess && message && !isLoading) this.showToast({ message, isSuccess });
      },
    },
  },
  methods: {
    async fetchBudgetData(date = new Date()) {
      ...
      try {
        ...
        logError(err);
        this.queryBudgetState = { isLoading: false, message: err.message, isSuccess: false };
      }
    },
    dateThAttrs(column) {
